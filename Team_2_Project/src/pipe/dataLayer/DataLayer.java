...
/** ArrayList for rate Parmameters objects.*/
   ...
   /** ArrayList containing all the SAMComponent objects  */
   private ArrayList componentsArray = null;
   
   /** ArrayList containing all the SAMportin objects*/
   private ArrayList portinArray = null;
   ...
   private Hashtable PortinMap = null;
...
//      System.out.println("inhibitorsMap size: " + inhibitorsMap.size());
   ...
   /**
    * serve for building net composition model when analysis the whole sam model.
    * @param pnObject
    */
   public void addPetriNetObjectToNetCompositionModel(PetriNetObject pnObject){
...
//               attached.updateConnected(); //causing null pointer exceptions (?)
   ...
   private Place createPlace(Element element){
      ...
      if(datatypename.length() > 0 && datatypeNtype.length() > 0 && datatypetypes.length() > 0
    		  && datatypeifPow.length() >0 && datatypeNumofElement.length() > 0 && datatypeisDef.length() > 0)
      {
      ...
      for(int i = 0; i < nodelist.getLength(); i ++)
      {
    	  ...
    	  if(item.getNodeName().equals("group-DataType"))
    	  {
	          ...
	          if(g_datatypename.length() > 0 && g_datatypeNtype.length() > 0 && g_datatypetypes.length() 				> 0&& g_datatypeifPow.length() >0 && g_datatypeNumofElement.length() > 0 && 				g_datatypeisDef.length() > 0)
	          {
...
//    System.out.println("Random Fired Transition Backwards" + ((Transition)transitionsArray.get(randonTransition)).getId());
   ...
   public void createFromPNML(Document PNMLDoc)	{
      ...
      try {
//         nodeList = PNMLDoc.getDocumentElement().getChildNodes().item(0).getChildNodes();
